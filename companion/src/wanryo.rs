...
pub fn run(cfg: &Config) -> Result<()> {
    ...
    let jobs = api::list_done_companion_jobs(&cfg.server_url)
        .context("fetch done jobs")?;

    let companion: Vec<_> = jobs.into_iter()
    ...
    for job in companion {
        ...
        let filename = source.file_name().and_then(|n| n.to_str())
            .unwrap_or("?").to_string();

        let source_exists = source.exists();
        let output_exists = output.exists();
        let source_size   = if source_exists { source.metadata().map(|m| m.len()).unwrap_or(0) } else { 0 };
    ...
    let needs_download:  Vec<&Entry> = entries.iter().filter(|e| e.source_exists && !e.output_exists).collect();
    let source_missing:  Vec<&Entry> = entries.iter().filter(|e| !e.source_exists && !e.output_exists).collect();

    println!("\nStatus:");
    println!("  {} already synced", synced.len());
    println!("  {} ready to download (source found, no AV1 beside it)", needs_download.len());
    ...
    if !needs_download.is_empty() {
        ...
        for e in &needs_download {
            println!("  {} ({:.2} GB output)", e.filename, e.output_size as f64 / 1e9);
        }
        ...
        if ans.trim().eq_ignore_ascii_case("y") {
            for (i, e) in needs_download.iter().enumerate() {
                ...
                match api::download_output(&cfg.server_url, &e.job_id, &e.output, &bar) {
                    Ok(_) => {
                        bar.finish_and_clear();
                        println!("ok ({:.2} GB)", e.output.metadata().map(|m| m.len()).unwrap_or(0) as f64 / 1e9);
                        downloaded.push(e);
                    }
                    Err(err) => {
                        bar.finish_and_clear();
    ...
    for e in &entries {
        ...
        let savings = if e.source_size > 0 && e.output_size > 0 {
            format!("{:.0}%", (1.0 - e.output_size as f64 / e.source_size as f64) * 100.0)
        } else {
            "-".to_string()
        };
        ...
        out.push_str(&format!(
            ...
            if e.output_exists || downloaded.iter().any(|d| d.job_id == e.job_id) { "yes" } else { "no" },
            status,
        ));
    }
