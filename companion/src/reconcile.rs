...
pub fn reconcile(cfg: &Config, local_files: &[VideoFile]) {
    ...
    let all_jobs = match api::list_done_companion_jobs(&cfg.server_url, cfg.auth_token.as_deref()) {
        Ok(j) => j,
        Err(e) => {
            warn!("Reconcile: cannot fetch server jobs: {}", e);
            return;
        }
    };
    ...
    for (fname, mut jobs) in by_filename {
        ...
        jobs.sort_by_key(|j| {
            ...
            if src.contains("/.transcode/uploads/") {
                1
            } else {
                0
            }
    ...
    for (fname, job) in to_process {
        ...
        let confident: Vec<PathBuf> = candidates
            .into_iter()
        ...
        match confident.len() {
            0 => info!(
                "Reconcile: '{}' — found by name but metadata doesn't match",
                fname
            ),
            1 => {
            ...
            _ => {
                warn!(
                    "Reconcile: '{}' — {} ambiguous local matches",
                    fname,
                    confident.len()
                );
                for p in &confident {
...
fn handle_match(cfg: Config, job: api::ServerJob, local_path: PathBuf) {
    ...
    if let Err(e) = api::set_client_path(
        &cfg.server_url,
    ...
    if output_path.exists() {
        info!(
            "Reconcile: output already exists at {} — updating local state only",
            output_path.display()
        );
        let _ = state::upsert(
            ...
            state::JobEntry {
                ...
                source_path: Some(local_path.to_string_lossy().to_string()),
                source_stats: None,
                output_stats: None,
        ...
        crate::platform::get_platform().notify(
            "Enkodu \u{2713}",
            &format!("Reconcile: linked {}  (already on disk)", name),
        );
        return;
    }
    ...
    loop {
        match api::poll_job(&cfg.server_url, cfg.auth_token.as_deref(), &job_id) {
    ...
    if let Err(e) = api::download_output_with_retry(
        ...
        crate::platform::get_platform().notify(
            "Enkodu \u{2717}",
            &format!("Reconcile download failed: {}", name),
        );
        return;
    }
    ...
    match verify::probe(&output_path) {
        Ok(info) if info.codec != "av1" => {
            ...
            crate::platform::get_platform().notify(
                "Enkodu \u{2717}",
                &format!("Bad codec after reconcile: {}", name),
            );
    ...
    crate::platform::get_platform().notify(
        "Enkodu \u{2713}",
        &format!("Reconcile: {}  ({:.2} GB)", name, out_sz as f64 / 1e9),
    );
}
